...
session_start();
    ...
    $query = "SELECT * FROM `case` WHERE (status='created' and heat='medium') or (status='Reported' and heat='medium') or (status='reinvestigate' and heat='medium') ORDER BY id DESC";
  ...
  <td><?php echo $rows['ran'];?></td>
   <td><?php echo $rows['createat'];?></td>
     ...
     <td><?php echo $rows['zipcode'];?></td>
      ...
      <?php if($rows['heat']=="medium")
      {

        ?>
       <td><button class="btn btn-danger" style="border-radius: 50px;height: 20px; padding: 1%;background-color: #F58711;" ><?php echo $rows['heat'];?></button></td>
     <?php }?>
      <td>
      <?php
    ...
    if($rows['status']=="Reported"){
     ...
     <?php } ?>
         ...
         <td><a href="case/testingindex.php?id=<?php echo $rows['ran'];?>" class="btn btn-primary" style="background-color: #034ea2;">Investigate</a></td>
