<?php
    ...
    $query = "SELECT * FROM `case` WHERE (status='created' and heat='high') or (status='Reported' and heat='high') or  (status='reinvestigate' and heat='high') ORDER BY id DESC";
   ...
   <td><?php echo $rows['createat'];?></td>
     ...
     <td><?php echo $rows['zipcode'];?></td>
      <td><?php echo $rows['phone'];?></td>
       <td><button class="btn btn-danger" style="border-radius: 50px;height: 20px; padding: 1%;" ><?php echo $rows['heat'];?></button></td>
      <td>
      <?php
    ...
    if($rows['status']=="Reported"){
     ...
     <?php } ?>
         ...
         <td><a href="case/hospitalindex.php?id=<?php echo $rows['ran'];?>" class="btn btn-primary" style="background-color: #034ea2;">Investigate</a></td>
